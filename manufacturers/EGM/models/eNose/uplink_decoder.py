...
def read_BME680_Res(BME680_Res_bytes):
    ...
    try:
        ...
        BME680_Res_uint32 = struct.unpack("<I", bytes(BME680_Res_bytes))[
            0
        ]  # <I pour uint32_t
    ...
    if (
        BME680_Res_uint32 > 10000000 or BME680_Res_uint32 < 0
    ):  # Seuil à ajuster si nécessaire
...
def ENose_Sensor(payload_bytes):
    ...
    if len(payload_bytes) < expected_length:
        raise Exception(
            f"The payload has an unexpected length: {len(payload_bytes)}. expected: {expected_length}."
        )
    ...
    eNose_json = json.dumps(
        {
            ...
            "Air_components": (
                Air_components
                if Air_components != 4095 and Air_components >= 0
                else None
            ),
            "Methane": Methane if Methane != 4095 and Methane >= 0 else None,
            "Air_contaminants": (
                Air_contaminants
                if Air_contaminants != 4095 and Air_contaminants >= 0
                else None
            ),
            "Butane_Propane": (
                Butane_Propane
                if Butane_Propane != 4095 and Butane_Propane >= 0
                else None
            ),
            "VOC_isobutane": (
                VOC_isobutane if VOC_isobutane != 4095 and VOC_isobutane >= 0 else None
            ),
            ...
            "Temperature": (
                Temperature if Temperature != 4095 and Temperature >= 0 else None
            ),
...
def ENose_BME680_Res(payload_bytes):
    ...
    for a in range(10):
        ...
        if BME680_Res_value is not None:
            ...
            warnings.warn(f"Rés {chr(65+a)} ignorée")
        i += 4
...
def decode_payload(payload_string, port_string):
    ...
    if port_string == "1":
        ...
        if decoded_data_json is None:
            warnings.warn(
                f"Warning: No valid data decoded for port 1. Payload: {payload_string}"
            )
            return "{}"
    ...
    if decoded_data_json == "{}" or decoded_data_json is None:
        warnings.warn(
            f"Warning: Invalid or empty data for port {port_string}. Payload: {payload_string}"
        )
        return "{}"
    ...
    ngsild_payload = [
        {
            "id": entity_id,
            "type": "Device",
            "ammonia": ngsild_instance(
                input["Ammonia"], input["timestamp"], "A99", "Raw"
            ),
            "airComponents": ngsild_instance(
                input["Air_components"], input["timestamp"], "A99", "Raw"
            ),
            "methane": ngsild_instance(
                input["Methane"], input["timestamp"], "A99", "Raw"
            ),
            "airContaminants": ngsild_instance(
                input["Air_contaminants"], input["timestamp"], "A99", "Raw"
            ),
            "butanePropane": ngsild_instance(
                input["Butane_Propane"], input["timestamp"], "A99", "Raw"
            ),
            "vocIsobutane": ngsild_instance(
                input["VOC_isobutane"], input["timestamp"], "A99", "Raw"
            ),
            "hydrogen": ngsild_instance(
                input["Hydrogen"], input["timestamp"], "A99", "Raw"
            ),
            "no2": ngsild_instance(input["NO2"], input["timestamp"], "A99", "Raw"),
            "co2": ngsild_instance(input["CO2"], input["timestamp"], "A99", "Raw"),
            "temperature": ngsild_instance(
                input["Temperature"], input["timestamp"], "CEL", "Raw"
            ),
            "pressure": ngsild_instance(
                input["Pressure"], input["timestamp"], "BAR", "Raw"
            ),
            "humidity": ngsild_instance(
                input["Humidity"], input["timestamp"], "P1", "Raw"
            ),
            "batteryVoltage": ngsild_instance(
                input["voltage"], input["timestamp"], "VLT", "Raw"
            ),
        }
    ]
    return ngsild_payload
...
def ngsild_instance(value, time, unitCode, dataset_suffix):
    ngsild_instance = {"type": "Property", "value": value, "observedAt": time}
...
def main():
    ...
    if decoded_json == "{}":
        warnings.warn(
            f"Warning: Invalid or empty data for port {fport}. Payload: {payload}",
        )
        # On renvoie un NGSI-LD vide
        json.dump([{"id": entity_id, "type": "Device"}], sys.stdout, indent=4)
        return
    ...
    if fport == "1":
        ...
        ngsild_payload = [{"id": entity_id, "type": "Device", **decoded}]
    else:
        ngsild_payload = [{"id": entity_id, "type": "Device"}]
