    ...
    if (error){
        ...
        for (let i = 0; i < messages.length; i++) {
            if (messages[i].type === 'report_telemetry') {
				...
				else if (messages[i].measurementId === 4205) {
                    ngsild_payload[0].epsilon = ngsildInstance(messages[i].measurementValue, time, null, 'Raw');
                }
				else if (messages[i].measurementId === 4204) {
